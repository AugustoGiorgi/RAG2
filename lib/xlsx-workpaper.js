...
function styleSheet(ws, sheet) {
  ...
  rows.forEach((rawRow, r) => {
    ...
    for (let c = 0; c < colCount; c++) {
      ...
      // Explicit formula cell: a code builder passed { formula, value }. The formula is
      // always written by code over cell addresses code itself placed, wrapped in IFERROR
      // with the code-computed value as fallback + cached result, so it can never show
      // #REF!/#ERROR. This is how the M-1 subtotals stay live when the preparer edits a line.
      if (raw && typeof raw === "object" && typeof raw.formula === "string") {
        const fallback = Number.isFinite(Number(raw.value)) ? Number(raw.value) : 0;
        cell.value = { formula: `IFERROR(${raw.formula},${fallback})`, result: fallback };
      ...
      // Source/verbatim sheets (faithful copies of the client's uploaded reports) keep
      // their totals as static numbers — we never rewrite a number inside the original.
      if (isTotal && num !== null && !sheet.verbatim) {
