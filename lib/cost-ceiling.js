 ...
 */
function inputTokenAllowance({ ceilingUsd, rates, outputTokens }) {
  if (!ceilingUsd || !(ceilingUsd > 0) || !rates) return null;
  const outputUsd = (Number(outputTokens || 0) / 1e6) * rates.outputPerMTok;
  const forInput = ceilingUsd - outputUsd;
  if (forInput <= 0) return 0;
  return Math.floor((forInput / rates.inputPerMTok) * 1e6);
}

/** Lo que costaria una corrida con estos tokens. */
function priceOf({ inputTokens, outputTokens, rates }) {
  if (!rates) return null;
  return (Number(inputTokens || 0) / 1e6) * rates.inputPerMTok
    + (Number(outputTokens || 0) / 1e6) * rates.outputPerMTok;
}
...
async function fitToCeiling({
  ceilingUsd, rates, maxOutputTokens, totalChars, perFileChars, build, count,
}) {
  ...
  for (; passes <= MAX_PASSES; passes += 1) {
    ...
    const allowance = inputTokenAllowance({ ceilingUsd, rates, outputTokens: MIN_OUTPUT_TOKENS });
    if (allowance === null) break;
    if (inputTokens <= allowance) {
      const room = Math.floor(((ceilingUsd - (inputTokens / 1e6) * rates.inputPerMTok) / rates.outputPerMTok) * 1e6);
  ...
  return {
    ...
    estimatedUsd: priceOf({ inputTokens, outputTokens, rates }),
    totalChars: total,
