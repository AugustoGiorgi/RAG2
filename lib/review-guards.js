...
const PRESENT_VERB = /(?:shows?|reports?|reflects?|carries|=|of|totaling|at)\s*$/i;
...
function assertedOnReturn(before) {
  if (!PRESENT_VERB.test(before)) return false;
  const at = before.search(RETURN_REFERENCE_NEAR);
...
function claimedAmounts(text) {
  ...
  for (const match of haystack.matchAll(AMOUNT_IN_TEXT)) {
    ...
    if (assertedOnReturn(haystack.slice(Math.max(0, match.index - 80), match.index))) continue;
...
const MISSING_FORM_CLAIM = /\b(?:not attached|are not attached|no longer attached|missing|not present|not visible|not found|not located|could not be located|not included|not in the (?:package|return)|not provided|not supplied|not furnished|does not include|omitted|not listed)\b/i;
/**
