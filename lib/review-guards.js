...
const ABSENCE_CLAIM = /\b(?:not reported|does not (?:appear|report|show|reflect)|do not (?:appear|report|show|reflect)|is not (?:reported|shown|included|reflected|present)|are not (?:reported|shown|included|reflected)|nowhere on|absent from)\b/i;
...
function verifyAbsenceClaims(review, files) {
  ...
  const checked = issues.map((issue) => {
    ...
    return {
      ...
      riskAnalysis: `CONTRADICTED BY THE RETURN: this finding says ${present.map(money).join(" and ")} ${present.length > 1 ? "are" : "is"} not reported, but the figure is printed on the return${where ? ` — "${where.slice(0, 180)}"` : ""}. Lowered to LOW automatically; read the cited line before acting on it. ${issue.riskAnalysis || ""}`.trim(),
    };
  });
