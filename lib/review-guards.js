...
const { amountAppearsInText, textCitesFile } = require("./tie-out");
...
function verifyWorkpaperClaims(review, files) {
  const issues = Array.isArray(review?.issues) ? review.issues : [];
  if (!issues.length) return { issues, corrected: 0 };
  // Solo los papeles del año QUE SE REVISA. Sin este filtro el guard juntaba los workpapers de
  // los dos años en un texto y buscaba ahi, de modo que una cifra del año anterior alcanzaba
  // para desactivar un hallazgo sobre el año corriente. Paso en produccion: la observacion
  // decia que la compensacion de directivos cayo de $75.000 en 2024 a $0 en 2025 —un tema de
  // reasonable compensation de una S-corp— y el guard encontro "Less: Jason Jeffery W-2 Salary
  // (75,000)" en el workpaper de 2024 y la bajo a LOW. La cifra estaba, pero en el año que la
  // observacion decia que estaba. verifyAttachmentClaims ya filtraba asi; este no.
  const support = (Array.isArray(files) ? files : []).filter((file) => {
    const role = String(file?.reviewRole || file?.role || "").toLowerCase();
    return !role.includes("return") && !role.includes("prior");
  });
...
/** A workbook row label; past this it is prose that happens to contain one of the words. */
const MAX_ADJUSTMENT_LABEL = 60;
...
function reconcilingLinesWithoutAmounts(files) {
  const empty = [];
  // One document at a time. A ZIP holds the whole support folder, so testing the archive for
  // "book to tax" passed on any of forty-one documents mentioning taxable income, and an
  // expense checklist got scanned as if it were a reconciliation: "VEHICLE, TRAVEL & MEALS"
  // came back as an adjustment somebody left blank. The question is per document, and so is
  // the name that gets reported.
  const documents = [];
  for (const file of Array.isArray(files) ? files : []) {
    if (!file) continue;
    const whole = String(file.fullText || file.text || "");
    const entries = zipEntries(whole);
    if (entries.length) { for (const entry of entries) documents.push({ name: entry.name, text: entry.text }); continue; }
    documents.push({ name: String(file.name || "workpaper"), text: whole });
  }
  for (const file of documents) {
    if (!isSpreadsheet(file)) continue;
    const text = file.text;
    if (!RECONCILIATION_CONTEXT.test(text)) continue;
    // Only rows that are genuinely inside a sheet. A client's support folder arrives as one
    // ZIP whose text is everything it contains, so a single embedded .xlsx made the entire
    // bundle look like a workbook and the scan ran across forty PDFs of tax boilerplate. The
    // label vocabulary includes "penalty", which is in the fine print of nearly every tax
    // form, and "Under penalties of perjury, I declare that I have examined this return" came
    // back reported as an adjustment somebody wrote down and left blank.
    let inSheet = !/^---\s/m.test(text);
    for (const raw of text.split(/\r?\n/)) {
      const marker = /^---\s*(.+?)\s*---$/.exec(raw.trim());
      if (marker) { inSheet = /^Sheet:/i.test(marker[1]); continue; }
      if (!inSheet) continue;
      ...
      // A reconciling row is labelled, not narrated. Anything sentence-length reached this
      // point by matching a word, not by being a row.
      if (!label || label.length > MAX_ADJUSTMENT_LABEL) continue;
      ...
      empty.push({ label: label.replace(/\s+/g, " ").trim(), source: file.name });
    }
  }
  return empty;
}
...
/* ---------------------------------------------------------------------------
 * 6. "The support does not show it" — from a review that opened a fraction of the support.
 *
 * A real 1040 arrived with forty-one supporting documents. The review produced four HIGH
 * findings, each of the same shape: wages of $1,470,470 against "W-3 total $247,200", a
 * $9,561,242 long-term gain with "no 1099-B support", Schedule E of $2,797,588 against "K-1
 * total $9,691". Every one of those differences was explained inside the package — by the two
 * W-2s it had not opened, by the partnership K-1 and Form 8308 for the sale, by twenty-eight
 * other K-1s. The review had read a handful of documents and reported the rest as absent.
 *
 * The signal used here is the one the model cannot influence: whether the review's own prose
 * NAMES a document. documentsRead is a self-report and a model will list a file it never
 * opened; a figure quoted from a file is evidence it was read.
 *
 * Deliberately hard to trigger. It needs a package big enough for "a fraction" to mean
 * something, a review that names less than half of it, and a finding that actually rests on
 * the support being absent. Anything less and a correct finding gets buried, which is the more
 * expensive mistake and the one this file has made before.
 * ------------------------------------------------------------------------- */

/** A finding whose weight rests on the supporting documents not showing something. */
const UNSUPPORTED_BY_PACKAGE = /\b(?:is unexplained|are unexplained|unexplained\b|not provided|no supporting|no support\b|without support|missing (?:k-?1|w-?2|1099|broker|statement)|no (?:visible )?(?:k-?1|w-?2|1099|broker|receipts|acknowledg\w+|worksheets?|schedules?|appraisal)\b)/i;
/** Under this there is no "fraction of the package" worth talking about. */
const COVERAGE_MIN_SUPPORT = 6;
/** Naming less than half of the readable support makes an absence claim a reading, not a fact. */
const COVERAGE_MIN_SHARE = 0.5;

/** The review's own words — never documentsRead, which is what the model says it opened. */
function reviewProse(review) {
  const parts = [];
  for (const issue of Array.isArray(review?.issues) ? review.issues : []) {
    parts.push(issue?.issueDescription, issue?.evidence, issue?.source, issue?.proposedSolution);
  }
  for (const item of Array.isArray(review?.verifiedItems) ? review.verifiedItems : []) parts.push(item);
  for (const item of Array.isArray(review?.openQuestions) ? review.openQuestions : []) parts.push(item);
  return parts.filter(Boolean).join(" \n ");
}

/** How a ZIP package names each file inside it once its text is concatenated. */
const ZIP_ENTRY = /^---\s*ZIP ENTRY:\s*(.+?)\s*---$/;
/** Below this an entry has no content worth counting as a document. */
const READABLE_MIN = 40;

/**
 * The documents inside a ZIP package.
 *
 * A client's support folder is uploaded as one archive and reaches the review as a single
 * file whose text is every document in it, one after another. Counting files therefore
 * counted one document where there were forty-one, which is how the coverage guard sat out
 * the very run it was written for.
 */
function zipEntries(text) {
  const out = [];
  let current = null;
  for (const line of String(text || "").split(/\r?\n/)) {
    const marker = ZIP_ENTRY.exec(line.trim());
    if (marker) {
      if (current && current.text.trim().length > READABLE_MIN) out.push(current);
      current = { name: marker[1], text: "" };
      continue;
    }
...
/** Supporting documents with readable text; the returns themselves are not "support". */
function readableSupport(files) {
  const support = [];
  for (const file of Array.isArray(files) ? files : []) {
    if (!file || !file.name) continue;
    if (/return/i.test(String(file.reviewRole || file.role || ""))) continue;
    const text = String(file.fullText || file.text || "");
    const entries = zipEntries(text);
    if (entries.length) { support.push(...entries); continue; }
    if (text.trim().length > READABLE_MIN) support.push({ name: file.name, text });
  }
  return support;
}

function verifySupportCoverage(review, files) {
  const issues = Array.isArray(review?.issues) ? review.issues : [];
  if (!issues.length) return { issues, corrected: 0 };

  const support = readableSupport(files);
  if (support.length < COVERAGE_MIN_SUPPORT) return { issues, corrected: 0 };

  const prose = reviewProse(review);
  const named = support.filter((file) => textCitesFile(prose, file.name));
  if (named.length / support.length >= COVERAGE_MIN_SHARE) return { issues, corrected: 0 };

  let corrected = 0;
  const checked = issues.map((issue) => {
    if (!issue || typeof issue !== "object") return issue;
    // Only findings that lean on the support being absent. A finding about the return
    // contradicting itself does not depend on how much of the package was opened.
    const claim = `${issue.issueDescription || ""} ${issue.evidence || ""} ${issue.source || ""}`;
    if (!UNSUPPORTED_BY_PACKAGE.test(claim)) return issue;
    corrected += 1;
    return {
      ...issue,
      priority: "LOW",
      contradictedByGuard: true,
      riskAnalysis: `COVERAGE: this finding rests on the supporting documents not showing something, and this review names ${named.length} of the ${support.length} readable supporting documents in the package. What is missing may be in the ${support.length - named.length} it does not cite. Lowered to LOW automatically; open the rest of the package before acting on it. ${issue.riskAnalysis || ""}`.trim(),
    };
  });
  return { issues: checked, corrected, named: named.length, support: support.length };
}


/* ---------------------------------------------------------------------------
 * 7. The model finding that repeats a deterministic one.
 *
 * As the deterministic library grew, the model started arriving at the same conclusions on
 * its own — and both went into the report. On a real 1065 the top of the list read "Schedule L
 * does not balance" twice, with the same three figures, and "the accounting method changed
 * with no Form 3115" twice. Two HIGH findings saying one thing makes a report of twenty look
 * careless, and the reader has to work out that they are the same item.
 *
 * Matched on two signals, never one: a pattern the deterministic check carries for its own
 * subject, AND a material figure the two findings share. Prose alone never matches — the model
 * writes its own words, which is why the first attempt at this, comparing sixty characters of
 * text, folded nothing in a year of running.
 *
 * Nothing is deleted. The repeat drops to LOW and says which finding it repeats, so a reader
 * who wants the model's wording still has it and the top of the report has one of each.
 * ------------------------------------------------------------------------- */

/** Below this a shared figure is a coincidence, not the same subject. */
const SHARED_FIGURE_FLOOR = 1000;
...
function materialFigures(text) {
  ...
  for (const match of String(text || "").matchAll(AMOUNT_IN_TEXT)) {
    ...
    if (Number.isFinite(value) && value >= SHARED_FIGURE_FLOOR) out.add(value);
  }
  return out;
}

/**
 * Cuantas cifras materiales compartidas alcanzan para decir que dos hallazgos son el mismo,
 * cuando no hay un patron escrito a mano.
 *
 * Dos. Una sola cifra en comun pasa seguido entre hallazgos distintos sobre la misma
 * declaracion — el ingreso ordinario aparece en media docena de lugares. Dos cifras de mas de
 * mil dolares coincidiendo ya no es casualidad: en la corrida que motivo esto, el hallazgo del
 * modelo sobre el Schedule M-1 repetia $63.319, $118.789,05 y $55.470 del cruce determinista.
 */
const SHARED_FIGURES_FOR_DUPLICATE = 2;

/**
 * El propio modelo dice que esta repitiendo: desde que los cruces del codigo le llegan como
 * hechos, devuelve hallazgos rotulados "Automated check: ..." que son la misma cosa con otras
 * palabras. Cuando el rotulo lo dice, alcanza con el tema mas parecido para plegarlo.
 */
const SELF_LABELED_ECHO = /^\s*automated(?!\s+cross-year check$)/i;
const ECHO_STOP = new Set("this that with from form line return which there their have been were year tax the and for not are its into over under than when what where each any all one two more most only also such being does did done off per via they them then those these while whose about after before above below between during out page part item case just like make made take taken give given show shows shown says said".split(" "));

function keyTokens(text) {
  const out = new Set();
  for (const word of String(text || "").toLowerCase().match(/[a-z0-9][a-z0-9.'-]{3,}/g) || []) if (!ECHO_STOP.has(word)) out.add(word);
  return out;
}

function closestSubject(text, all) {
  const tokens = keyTokens(text);
  let best = null;
  for (const subject of all) {
    const theirs = keyTokens(`${subject.title} ${subject.detail}`);
    if (!theirs.size) continue;
    let shared = 0;
    for (const token of theirs) if (tokens.has(token)) shared += 1;
    const ratio = shared / theirs.size;
    if (!best || ratio > best.ratio) best = { subject, ratio };
  }
  return best && best.ratio >= 0.25 ? best.subject : null;
}

function foldFindingsRepeatedBy(issues, deterministic) {
  const list = Array.isArray(issues) ? issues : [];
  const all = (Array.isArray(deterministic) ? deterministic : []).filter(Boolean);
  if (!list.length || !all.length) return { issues: list, folded: 0 };

  let folded = 0;
  const checked = list.map((issue) => {
    if (!issue || typeof issue !== "object") return issue;
    if (String(issue.source || "") === "Automated cross-year check") return issue;
    const text = `${issue.formOrSchedule || ""} ${issue.issueDescription || ""} ${issue.evidence || ""}`;
    const figures = materialFigures(text);
    if (SELF_LABELED_ECHO.test(String(issue.source || "")) || /^\s*automated check\b/i.test(String(issue.issueDescription || ""))) {
      const subject = closestSubject(text, all);
      if (subject) {
        folded += 1;
        return {
          ...issue,
          priority: "LOW",
          contradictedByGuard: true,
          riskAnalysis: `REPEATS AN AUTOMATED FINDING: the review labelled this one of the automated checks, and it says the same thing as "${subject.title}", which is computed from the forms and appears above. Lowered to LOW automatically so the item is stated once. ${issue.riskAnalysis || ""}`.trim(),
        };
      }
    }
    const repeat = all.find((subject) => {
      const theirs = materialFigures(subject.detail);
      let shared = 0;
      for (const value of figures) if (theirs.has(value)) shared += 1;
      // Con patron escrito a mano alcanza una cifra: el patron ya fijo el tema.
      if (subject.dedupe && subject.dedupe.test(text)) return shared >= 1;
      // Sin patron, mandan las cifras. Antes esto no existia y solo 2 de los 39 cruces
      // deterministas tenian patron, asi que el duplicado del M-1 —mismo tema, mismas tres
      // cifras— salia igual en el informe, una vez como HIGH y otra como MEDIUM.
      return shared >= SHARED_FIGURES_FOR_DUPLICATE;
    });
    if (!repeat) return issue;
    folded += 1;
    return {
      ...issue,
      priority: "LOW",
      contradictedByGuard: true,
      riskAnalysis: `REPEATS AN AUTOMATED FINDING: this says the same thing as "${repeat.title}", which is computed from the forms and appears above with the same figures. Lowered to LOW automatically so the item is stated once; the wording here is kept in case it adds anything. ${issue.riskAnalysis || ""}`.trim(),
    };
  });
  return { issues: checked, folded };
}
...
/** Lo que dice un hallazgo cuando afirma que una depreciacion esta mal calculada. */
const DEPRECIATION_DOUBT = /\b(?:understat\w*|overstat\w*|does not match|doesn.t match|appears? (?:incorrect|understated|overstated|wrong)|recompute|does not (?:tie|reconcile)|inconsistent with)\b/i;
const DEPRECIATION_SUBJECT = /\bdepreciation\b|\bform 4562\b|\bmacrs\b|\bsection 179\b/i;

/**
 * Baja de categoria un hallazgo que dice que una depreciacion esta mal cuando la aritmetica
 * dice que esta bien.
 *
 * El caso real: el informe reporto como HIGH que $1.285 de depreciacion "parecen subestimados".
 * La cuenta es $4.016 de base x 32% —año dos de cinco, 200DB, medio año— = $1.285,12. El
 * modelo habia leido la base depreciable como depreciacion acumulada y se habia inventado una
 * base de $3.213; en la misma observacion recomendaba recalcular "sobre la base de $4.016",
 * contradiciendo su propio calculo.
 *
 * A diferencia de los otros guards, este no compara contra un documento sino contra una tabla:
 * lo que desmiente al hallazgo es el Rev. Proc. 87-57. Por eso la nota muestra la cuenta.
 */
function verifyDepreciationClaims(review, verified) {
  const issues = Array.isArray(review?.issues) ? review.issues : [];
  const rows = Array.isArray(verified) ? verified : [];
  if (!issues.length || !rows.length) return { issues, corrected: 0 };

  let corrected = 0;
  const checked = issues.map((issue) => {
    if (!issue || typeof issue !== "object") return issue;
    if (String(issue.source || "") === "Automated cross-year check") return issue;
    const claim = `${issue.formOrSchedule || ""} ${issue.issueDescription || ""} ${issue.evidence || ""} ${issue.proposedSolution || ""}`;
    if (!DEPRECIATION_SUBJECT.test(claim) || !DEPRECIATION_DOUBT.test(claim)) return issue;
    const figures = materialFigures(claim);
    const match = rows.find((row) => figures.has(Math.abs(row.claimed)) || figures.has(Math.abs(row.basis)));
    ...
    return {
      ...
      priority: "LOW",
      contradictedByGuard: true,
      riskAnalysis: `RECOMPUTED AND CORRECT: the app recalculated this deduction from the figures printed on Form 4562 Part V — ${money(match.basis)} of depreciable basis, year ${match.yearIndex} of a ${match.recovery}-year ${match.method} half-year schedule — which allows ${money(match.expected)}, and the return deducts ${money(match.claimed)}. The two agree. Lowered to LOW automatically; the arithmetic is above if you want to check it. ${issue.riskAnalysis || ""}`.trim(),
    };
  });
  return { issues: checked, corrected };
}
...
module.exports = {
  ...
  verifyDepreciationClaims,
  formAppearsInPackage,
  verifyContinuityClaims,
  verifySupportCoverage,
  foldFindingsRepeatedBy,
  readableSupport,
  zipEntries,
