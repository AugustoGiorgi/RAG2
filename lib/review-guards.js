...
const { csvCells } = require("./entity-return-checks");
...
const MISSING_FORM_CLAIM = /\b(?:not attached|is not attached|are not attached|no longer attached|missing|not present|not visible|not found|not located|could not be located|not included|not in the (?:package|return)|was not provided|does not include|omitted|not listed)\b/i;
/** "Schedule B-1", "Form 8825", "Schedule K-2" — the name as the finding writes it. */
const FORM_NAME = /\b(schedule|form)\s+([a-z]{1,2}-\d{1,2}|\d{3,4}-?[a-z]?|[a-z]-\d)\b/gi;
...
function formAppearsInPackage(kind, number, text) {
  ...
  const named = kind.toLowerCase() === "form"
    ? `(?:form)\\s*${escaped}`
  ...
  // 1125-A, 6765, 8879-PE" — so the "Form" is optional here. Safe because the pattern is
  // anchored to a line that opens as a forms list; a bare number anywhere else is not.
  const inFormsList = new RegExp(`^\\s*(?:federal|state|new york|forms? needed)[^\\n]*\\b(?:(?:sch\\.?|schedule|form)\\s*)?${escaped}\\b`, "im");
...
function verifyAttachmentClaims(review, files) {
  const issues = Array.isArray(review?.issues) ? review.issues : [];
  if (!issues.length) return { issues, corrected: 0 };
  const text = (Array.isArray(files) ? files : [])
  ...
  const checked = issues.map((issue) => {
    ...
    const present = [];
    for (const [, kind, number] of claim.matchAll(FORM_NAME)) {
...
function verifyWorkpaperClaims(review, files) {
  ...
  const checked = issues.map((issue) => {
    ...
    const present = [...new Set(claimedAmounts(claim))].filter((amount) => amountAppearsInText(amount, text));
    if (!present.length) return issue;
    corrected += 1;
