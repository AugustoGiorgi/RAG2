...
function foldFindingsRepeatedBy(issues, deterministic) {
  ...
  const subjects = (Array.isArray(deterministic) ? deterministic : []).filter((f) => f && f.dedupe);
  if (!list.length || !subjects.length) return { issues: list, folded: 0 };
  ...
  const checked = list.map((issue) => {
    ...
    const repeat = subjects.find((subject) => {
      if (!subject.dedupe.test(text)) return false;
      const theirs = materialFigures(subject.detail);
      for (const value of figures) if (theirs.has(value)) return true;
      return false;
    });
