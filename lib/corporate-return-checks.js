...
const { amountsOn, parseAmount, splitReturns, returnTextOf, linesOf } = require("./entity-return-checks");
...
/* ---------------------------------------------------------------------------
 * 2. Amortization on the return that the books never recorded.
 *
...
/* ---------------------------------------------------------------------------
 * 3. Shareholder loans filed as though they were trade payables.
 *
...
/* ---------------------------------------------------------------------------
 * 4. A loan the corporation made to its own shareholder.
 *
...
/* ---------------------------------------------------------------------------
 * 5. Interest accrued to a shareholder and never paid.
 *
...
/* ---------------------------------------------------------------------------
 * 7. A shareholder's stock basis that does not open where the prior year closed.
 *
 * Form 7203 line 15 closes the year and line 1 of the next return opens it, and nothing sits
 * between them: the same number, written twice, a year apart. When they differ, either a
 * basis adjustment went unrecorded or the two returns disagree about what the shareholder owns.
 *
 * This check exists because a review invented the answer. Reading an 1120-S with two
 * shareholders, it compared one shareholder's opening basis against the OTHER's prior-year
 * close, found an $17 gap it had created by crossing them, wrote "less $17 rounding" to
 * explain it away, and filed the whole thing under verified items. Both shareholders tie
 * exactly. Matching on the identifying number is the entire difficulty, so it belongs in code:
 * a fabricated reconciliation reads exactly like a real one.
 * ------------------------------------------------------------------------- */

const SHAREHOLDER_HEADER = /^Name of shareholder\b/i;
const TAXPAYER_ID = /\b(\d{3}-\d{2}-\d{4})\b/;
const BASIS_OPENING = /^\s*1\s+Stock basis at the beginning of the corporation/i;
const BASIS_CLOSING = /^\s*15\s+Stock basis at the end of the corporation/i;
/** Line 15 wraps, and its figure prints at the end of the continuation: "... 15 123,197." */
const CLOSING_FIGURE = /\b15\s+([\d,]+)\.?\s*$/;
/** Returns are whole-dollar at both ends; a dollar is not a break in continuity. */
const BASIS_TOLERANCE = 1;
/** A Form 7203 runs about two pages; past that the next shareholder has started. */
const FORM_7203_REACH = 260;

/** Every Form 7203 in a package: who it belongs to, and the basis at each end of the year. */
function form7203Basis(text) {
  const lines = linesOf(text);
  const starts = [];
  lines.forEach((line, index) => { if (SHAREHOLDER_HEADER.test(line)) starts.push(index); });

  const accounts = [];
  for (let i = 0; i < starts.length; i += 1) {
    const from = starts[i];
    const to = i + 1 < starts.length ? Math.min(starts[i + 1], from + FORM_7203_REACH) : Math.min(lines.length, from + FORM_7203_REACH);
    const block = lines.slice(from, to);
    const heading = block.slice(0, 3).join(" ");
    const id = (heading.match(TAXPAYER_ID) || [])[1];
    if (!id) continue;
    const name = (heading.match(/\b([A-Z][A-Z'.-]+(?:\s+[A-Z][A-Z'.-]+)+)\s+\d{3}-\d{2}-\d{4}/) || [])[1] || null;

    let beginning = null;
    let closing = null;
    for (let n = 0; n < block.length; n += 1) {
      if (beginning === null && BASIS_OPENING.test(block[n])) {
        const values = amountsOn(block[n]);
        const last = values.length ? values[values.length - 1] : null;
        // An empty line prints only its own box number; "1" is not an opening basis.
        if (last !== null && last !== 1) beginning = last;
      }
      if (closing === null && BASIS_CLOSING.test(block[n])) {
        for (const candidate of block.slice(n, n + 3)) {
          const hit = CLOSING_FIGURE.exec(candidate);
          if (hit) { closing = parseAmount(hit[1]); break; }
        }
      }
    }
    if (beginning !== null || closing !== null) accounts.push({ id, name, beginning, closing });
  }
  return accounts;
}

function checkShareholderBasisContinuity(currentText, priorText, priorYearLabel) {
  if (!priorText) return null;
  const current = form7203Basis(currentText);
  const prior = form7203Basis(priorText);
  if (!current.length || !prior.length) return null;

  const closed = new Map(prior.filter((a) => a.closing !== null).map((a) => [a.id, a.closing]));
  const breaks = [];
  for (const account of current) {
    if (account.beginning === null || !closed.has(account.id)) continue;
    const ended = closed.get(account.id);
    const gap = round2(account.beginning - ended);
    if (Math.abs(gap) <= BASIS_TOLERANCE) continue;
    breaks.push({ who: account.name || `the shareholder ending ${account.id.slice(-4)}`, ended, opened: account.beginning, gap });
  }
  if (!breaks.length) return null;

  const named = breaks
    .map((b) => `${b.who}'s basis closed ${priorYearLabel} at ${money(b.ended)} and opens at ${money(b.opened)}, out by ${money(b.gap)}`)
    .join("; ");
  return {
    severity: "HIGH",
    category: "Shareholder basis",
    title: "A shareholder's stock basis does not open where the prior year closed",
    detail: `${named}. Line 15 of one year's Form 7203 and line 1 of the next are the same figure written twice, with nothing in between to explain a difference.`,
    action: "Trace the shareholder's basis across the two returns. A gap usually means a distribution, a capital contribution or a nondeductible expense reached one year and not the other — and whichever way it goes, the shareholder is carrying the wrong number into every later year, including the one that decides whether a loss is deductible at all.",
    authority: "IRC §1367 (adjustments to basis); IRC §1366(d) (loss limited to basis); Form 7203 lines 1 and 15",
  };
}
...
function runCorporateReturnChecks(files, meta = {}) {
  ...
  return [
    ...
    checkShareholderBasisContinuity(text, prior ? returnTextOf(prior) : null, priorLabel),
  ].filter(Boolean);
}
...
module.exports = {
  ...
  checkShareholderBasisContinuity,
  form7203Basis,
