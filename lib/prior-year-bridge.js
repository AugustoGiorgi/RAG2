...
function splitReturns(files) {
  const list = (Array.isArray(files) ? files : []).filter((f) => f && String(f.text || "").trim().length > 500);
  const roleOf = (f) => String(f.reviewRole || f.role || "").toLowerCase();
  return {
    current: list.find((f) => roleOf(f).includes("current_return")) || null,
    prior: list.find((f) => roleOf(f).includes("prior_return")) || null,
  };
}
...
function runPriorYearChecks(files, meta = {}) {
  const { current, prior } = splitReturns(files);
  if (!current) return [];
  const currentText = String(current.text || "");
  const priorText = prior ? String(prior.text || "") : "";
