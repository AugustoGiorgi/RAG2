...
function depreciationDeducted(text) {
  ...
  if (rental) {
    // An empty line prints its own box number and nothing else, and summing that reported $14
    // of depreciation on a Form 8825 that claimed none. The federal branch below has guarded
    // against exactly this since the New York IT-204 caused it; the rental branch never did,
    // so the same bug survived one line above its own fix.
    const boxNumber = Number((rental.line.match(/^\s*(\d{1,2})/) || [])[1]);
    const values = rental.values.length === 1 && rental.values[0] === boxNumber ? [] : rental.values;
    return values.length ? round2(values.reduce((sum, n) => sum + n, 0)) : null;
  }
  ...
  return federalDeductionAmount(text, DEPRECIATION_DEDUCTION_LINE);
}

/** The deduction lines, allowing the rotated margin letters that sit in front of them. */
const DEPRECIATION_DEDUCTION_LINE = /^(?:[A-Z] )*\s*(?:16\s*a|20|14)\s+Depreciation\b(?!.*amortization)/i;
/** A figure a form actually prints: decorated with a comma or a decimal, never a bare index. */
const PRINTED_FIGURE = /(\d{1,3}(?:,\d{3})+(?:\.\d+)?|\d+\.\d{2})\.?\s*$/;
/** A wrapped continuation carries margin letters and a figure, nothing else. */
const MARGIN_CONTINUATION = /^(?:[A-Z] ?)+\s*[\d,]+\.?\s*$/;

/**
 * The amount on a Form 1065 deduction line.
 *
 * The word DEDUCTIONS is printed down the left margin of the form, and pdf.js hands each of
 * its letters to the nearest text row. The depreciation line therefore arrives as
 * "U O 16 a Depreciation (if required, attach Form 4562)" — an anchor of ^\s*16 never matches
 * it — and the amount lands on the FOLLOWING line as "C R 53,195.". Reading only the label
 * line returned null for both years of a partnership that deducted $53,195 in one of them, so
 * the check that depended on it never once fired on a partnership return.
 */
function federalDeductionAmount(text, pattern) {
  const lines = linesOf(text);
  const at = lines.findIndex((line) => pattern.test(line));
  if (at === -1) return null;

  const onLabel = amountsOn(lines[at]);
  if (onLabel.length) {
    const value = onLabel[onLabel.length - 1];
    const boxNumber = Number((lines[at].replace(/^(?:[A-Z] )*/, "").match(/^\s*(\d{1,2})/) || [])[1]);
    if (value !== boxNumber) return value;
  }
  // The continuation, and only if it is one: margin letters and a figure. Anything longer is
  // the next line of the form, whose own box number would read as an amount.
  const next = String(lines[at + 1] || "").trim();
  if (!MARGIN_CONTINUATION.test(next)) return null;
  const printed = PRINTED_FIGURE.exec(next);
  return printed ? parseAmount(printed[1]) : null;
}
...
function checkBalanceSheetBalances(currentText) {
  ...
  return {
    ...
    // El modelo llega solo a esta misma conclusion y las dos entraban al informe.
    dedupe: /schedule l (?:does not|doesn.t) balance|balance sheet does not balance|total assets[^.]{0,40}total liabilities/i,
...
/* ---------------------------------------------------------------------------
 * Los porcentajes de participacion de los K-1 no suman 100%.
 *
 * Cada K-1 imprime la parte del socio en ganancias, perdidas y capital, y las tres columnas
 * tienen que cerrar en 100 entre todos los socios. Es aritmetica, no criterio: si no cierran,
 * o falta un K-1 o alguno tiene el porcentaje mal cargado, y en cualquiera de los dos casos la
 * asignacion de resultados que sale de ahi esta mal.
 *
 * El resguardo importante es de extraccion, no de impuestos. Sobre una declaracion real el
 * extractor devolvio UN porcentaje de ganancia donde habia dos socios, y la suma dio 50% — un
 * hallazgo que no existe. Por eso la cantidad de porcentajes leidos tiene que coincidir con la
 * cantidad de K-1 que la propia declaracion declara en su linea I; si no coincide, el problema
 * es la lectura y no la declaracion, y no se opina.
 * ------------------------------------------------------------------------- */

/** "Profit 64.42 % 64.42 %" — apertura y cierre; se toma el primero, que es el del año. */
const K1_SHARE_LINE = /^\s*(Profit|Loss|Capital)\s+([0-9]+(?:\.[0-9]+)?)\s*%/;
/** Linea I del 1065 / 1120-S: cuantos K-1 se adjuntan. */
// El hueco entre los dos puntos y el numero es la columna del formulario: en las declaraciones
// medidas llega a 180 espacios, asi que el limite tiene que ser generoso.
const K1_COUNT_LINE = /Number of Schedules K-1\.?\s*Attach one for each person who was a (?:partner|shareholder)[^0-9]{0,400}(\d{1,4})/i;
/** El formulario imprime dos decimales; por debajo de esto es redondeo del software. */
const SHARE_TOLERANCE = 0.05;

function declaredK1Count(text) {
  const hit = K1_COUNT_LINE.exec(String(text || ""));
  if (!hit) return null;
  const value = Number(hit[1]);
  return Number.isFinite(value) && value > 0 ? value : null;
}

function k1Shares(text) {
  const byLabel = { Profit: [], Loss: [], Capital: [] };
  for (const line of String(text || "").split(/\r?\n/)) {
    const hit = K1_SHARE_LINE.exec(line);
    if (!hit) continue;
    const value = Number(hit[2]);
    if (Number.isFinite(value)) byLabel[hit[1]].push(value);
  }
  return byLabel;
}

function checkK1SharesFootTo100(currentText) {
  const declared = declaredK1Count(currentText);
  if (!declared) return null;
  const shares = k1Shares(currentText);

  const off = [];
  for (const [label, values] of Object.entries(shares)) {
    // Si no se leyo un porcentaje por cada K-1, el que fallo fue el extractor.
    if (values.length !== declared) continue;
    const sum = values.reduce((a, b) => a + b, 0);
    if (Math.abs(sum - 100) > SHARE_TOLERANCE) off.push({ label, sum, count: values.length });
  }
  if (!off.length) return null;

  const list = off.map((o) => `${o.label.toLowerCase()} adds to ${o.sum.toFixed(2)}%`).join(", ");
  return {
    severity: "HIGH",
    category: "Ownership percentages",
    title: "The Schedule K-1 ownership percentages do not add to 100%",
    detail: `Across the ${declared} Schedules K-1 attached to this return, ${list}. Every column has to close at 100% between all the owners.`,
    action: "Find the K-1 whose percentage is wrong, or the one that is missing. The allocation of income, deductions and credits is computed from these percentages, so every K-1 in the return is affected by whichever one is out.",
    authority: "Schedule K-1 (Form 1065) item J / Schedule K-1 (Form 1120-S) item F; the shares must total 100%",
    dedupe: /ownership percentage|k-?1 percentage|profit.{0,20}loss.{0,20}capital|do not add to 100/i,
  };
}
...
function runEntityReturnChecks(files, meta = {}) {
  ...
  const findings = [
    ...
    checkK1SharesFootTo100(currentText),
  ];
