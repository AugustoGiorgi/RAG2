...
const norm = (value) => String(value || "").toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 40);
...
function mergeReviews(reviews) {
  ...
  return {
    review: {
      ...base,
      issues,
      checkboxReview: mergeRows(list.map((r) => r.checkboxReview), (row) => norm(row.box)),
      infoConsistency: mergeRows(list.map((r) => r.infoConsistency), (row) => norm(row.item)),
