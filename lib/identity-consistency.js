...
/* ---------------------------------------------------------------------------
 * Las filas de infoConsistency que genera el CODIGO y no el modelo.
 *
 * Por que: la tabla de identificadores la escribia el modelo entera, fila por fila, y una fila
 * que dice "MATCH" es una afirmacion que puede estar mal — en una corrida documentada dos de
 * ellas lo estaban. El estudio quiere la tabla completa de vuelta, y eso es razonable: querer
 * ver que se verifico no es lo mismo que querer que alguien lo afirme sin mirar.
 *
 * La salida es: lo que se puede comparar con una comparacion de texto lo compara el codigo, y
 * esas filas no pueden mentir. Lo demas sigue siendo del modelo, que es lo unico capaz de leer
 * una casilla arbitraria en un formulario arbitrario.
 * ------------------------------------------------------------------------- */

/** El nombre del contribuyente: lo que se repite pegado a su numero en los encabezados. */
function dominantName(text, identifier) {
  if (!identifier) return null;
  const escaped = String(identifier).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const pattern = new RegExp("^(.{3,80}?)\\s+" + escaped + "\\b", "gm");
  const counts = new Map();
  for (const hit of String(text || "").matchAll(pattern)) {
    let name = hit[1].trim()
      // Los encabezados traen el formulario delante del nombre: "Form 1065 (2025) ACME LLC".
      .replace(/^(?:Form|Schedule|Sch\.?)\s+[\dA-Z-]+(?:\s*\([^)]*\))?\s*/i, "")
  ...
  if (!counts.size) return null;
  const ranked = [...counts.entries()].sort((a, b) => b[1] - a[1]);
  if (ranked[0][1] < IDENTIFIER_MIN_COUNT) return null;
  const runnerUp = ranked.length > 1 ? ranked[1][1] : 0;
  if (runnerUp > 0 && ranked[0][1] < runnerUp * IDENTIFIER_MIN_RATIO) return null;
  return { value: ranked[0][0], count: ranked[0][1] };
}

const COMPUTED_SOURCE = "Computed by RAG Tax AI from the filed returns";

/**
 * Filas de infoConsistency calculadas, no afirmadas.
 *
 * Fail-closed: lo que no se puede leer con margen no produce fila. Una fila que falta se nota;
 * una fila que dice MATCH sin haber mirado, no.
 */
function identityRows(files, meta = {}) {
  ...
  const currentYear = returnYear(current);
  const priorYearValue = prior ? returnYear(prior) : null;
  const statedMatch = String(meta.taxYear || "").match(/\d{4}/);
  const stated = statedMatch ? Number(statedMatch[0]) : null;

  if (currentYear !== null && stated !== null) {
    rows.push({
      item: "Tax year on the return",
      returnValue: String(currentYear),
      sourceValue: String(stated),
      source: "Tax year selected for this review",
      status: currentYear === stated ? "MATCH" : "MISMATCH",
      note: currentYear === stated
        ? "The year printed on the return is the year this review was run for."
        : `The review was run for ${stated} and the return filed is for ${currentYear}.`,
    });
  }

  if (currentYear !== null && priorYearValue !== null) {
    const gap = currentYear - priorYearValue;
    rows.push({
      item: "Prior-year return is the immediately preceding year",
      returnValue: String(currentYear),
      sourceValue: String(priorYearValue),
      source: (prior && prior.name) || "prior_return",
      status: gap === 1 ? "MATCH" : "MISMATCH",
      note: gap === 1
        ? "The two returns are one year apart, so every year-over-year comparison in this review compared consecutive years."
        : `The returns are ${Math.abs(gap)} year(s) apart. Every year-over-year comparison in this review is unreliable — mismatch.`,
    });
  }

  const currentId = dominantIdentifier(currentText);
  const priorId = priorText ? dominantIdentifier(priorText) : null;
  if (currentId && priorId) {
    const same = currentId.value === priorId.value;
    rows.push({
      item: "Taxpayer identifying number (SSN/EIN)",
      returnValue: masked(currentId.value),
      sourceValue: masked(priorId.value),
      source: (prior && prior.name) || "prior_return",
      status: same ? "MATCH" : "MISMATCH",
      note: same
        ? `The same identifying number heads both returns (${currentId.count} occurrences this year).`
        : "The two returns carry different identifying numbers — mismatch. One of them belongs to another taxpayer, or a digit is wrong.",
    });
  }

  const currentName = dominantName(currentText, currentId && currentId.value);
  const priorName = priorText ? dominantName(priorText, priorId && priorId.value) : null;
  if (currentName && priorName) {
    const same = currentName.value === priorName.value;
    rows.push({
      item: "Taxpayer / entity name",
      returnValue: currentName.value,
      sourceValue: priorName.value,
      source: (prior && prior.name) || "prior_return",
      status: same ? "MATCH" : "MISMATCH",
      note: same
        ? "The name printed on both returns is character-for-character the same."
        : "The name differs from last year — mismatch. A legitimate change (marriage, entity rename) still has to agree with what the IRS holds, because the e-file name control is built from it.",
    });
  }
...
/** Los items que el codigo ya cubre; el modelo no debe gastar una fila en ellos. */
const COMPUTED_ITEMS = [
  "Tax year on the return",
  "Prior-year return is the immediately preceding year",
  "Taxpayer identifying number (SSN/EIN)",
