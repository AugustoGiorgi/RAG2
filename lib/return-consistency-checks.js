...
// El 1065 y el 1120-S preguntan "would require YOU to file"; el 1120 pregunta "would require IT
// to file" en la linea 15a del Schedule K. Con el patron viejo, un 1120 pasaba por "formulario
// que no pregunta nada" y salia un hallazgo sobre una pregunta que la declaracion ya responde.
const FORM_1099_QUESTION = /(?:payments|payment).{0,60}would require (?:you|it|the corporation|the partnership|the S corporation) to file Form\(s\)\s*1099/i;
/** La segunda mitad de la pregunta: si los 1099 se presentaron o se van a presentar. */
const FORM_1099_FILED_QUESTION = /did (?:or will )?(?:you|it|the corporation|the partnership) (?:or will (?:you|it) )?file (?:the )?required Form\(s\)\s*1099/i;
...
function checkPaymentsRequiring1099(text) {
  ...
  if (!question) return null;
  // La declaracion admite las dos cosas: que los 1099 hacian falta y que no se presentaron.
  const filed = lineMatching(text, FORM_1099_FILED_QUESTION);
  if (answerOn(question) === "yes" && filed && answerOn(filed) === "no") {
    return {
      severity: "HIGH",
      category: "Information returns",
      title: "The return says the required 1099s were not filed",
      detail: "The return answers Yes to having made payments that require Forms 1099 and No to having filed them or intending to file them.",
      action: "File the 1099-NECs now — late is cheaper than not filed — and change the answer with them. As filed, the return is a written admission of a failure to file, at $340 per form with no cap for intentional disregard.",
      authority: "IRC §6041 and §6041A; IRC §6721 and §6722",
    };
  }
  if (answerOn(question) !== "no") return null;
...
function checkAccountingMethodChanged(currentText, priorText, priorLabel) {
  ...
  return {
    severity: "HIGH",
    category: "Accounting method",
    // Idem: "cambio el metodo contable y no hay 3115" es una conclusion que el modelo saca solo.
    dedupe: /accounting method[^.]{0,60}chang|chang[^.]{0,40}accounting method|form 3115/i,
...
/** The tax year a workbook belongs to, from its file name first and its own text second. */
function workpaperYear(file, text, taxYear) {
  if (!taxYear) return null;
  const name = String((file && file.name) || "");
  if (name.includes(taxYear)) return "named";
  return new RegExp(`\\b${taxYear}\\b`).test(text) ? "mentioned" : null;
}

/**
 * The workpaper's own bottom line, from the year being filed.
 *
 * This used to return whichever candidate came first in the file array. A package carrying
 * both this year's workpaper and last year's — which is every package with a prior-year
 * comparison — produced a different figure depending on upload order. A real 1120-S review
 * cited last year's $127,382 against this year's return and put a difference of -$64,063 in a
 * HIGH finding; the right figures were $118,789.05 and -$55,470.05. The same document's
 * tie-out had the correct number, because the model had read the right file.
 *
 * With more than one candidate, the year has to decide, and it has to decide unambiguously:
 * a guess here does not produce a missing finding, it produces a wrong number in a HIGH one.
 */
function bookNetIncome(files, taxYear) {
  const candidates = [];
  for (const file of Array.isArray(files) ? files : []) {
    ...
    for (const line of linesOf(text)) {
      ...
      if (!values.length) continue;
      candidates.push({ value: values[0], year: workpaperYear(file, text, taxYear) });
      break;
    }
  }
  if (candidates.length === 1) return candidates[0].value;
  if (!candidates.length) return null;
  // The file name is the one part of a workbook nobody edits by accident, so it goes first.
  for (const level of ["named", "mentioned"]) {
    const matched = candidates.filter((candidate) => candidate.year === level);
    if (matched.length === 1) return matched[0].value;
  }
  return null;
}

function checkScheduleM1TiesToBooks(text, files, taxYear) {
  const line = lineMatching(text, M1_LINE_1);
  if (!line) return null;
  const values = amountsOn(line);
  if (!values.length) return null;
  const stated = values[0];
  const books = bookNetIncome(files, taxYear);
...
/* ---------------------------------------------------------------------------
 * 12. A specified service business claiming a QBI deduction it cannot have.
 *
 * Above the income ceiling, section 199A(d)(3) excludes a specified service trade or business
 * entirely — not limited by wages, not phased: its QBI, its W-2 wages and its UBIA all drop
 * out. The distinction is therefore worth the whole deduction, and it is made by one checkbox
 * in column (b) of Form 8995-A Part I that nothing else on the return depends on.
 *
 * On a real 1040 with $13,505,921 of taxable income before QBI, "RETINA VITREOUS ASSOCIATE
 * MEDICAL GROUP" sat in Part I with that box unchecked and produced $393,278 of the return's
 * $400,165 deduction. Schedule A of Form 8995-A — the schedule that exists to handle specified
 * service businesses — was not in the package at all.
 *
 * The ceiling is read from the form's own printed labels rather than a table in this file, so
 * the check does not need updating each year, and the higher of the two printed ceilings is
 * used so filing status never has to be guessed.
 * ------------------------------------------------------------------------- */

const QBI_PART_I = /^Part I\s+Trade, Business, or Aggregation Information/i;
/** A Part I row: the column letter, the name, then the taxpayer identification number. */
const QBI_BUSINESS_ROW = /^\s*([A-C])\s+(\S.*?)\s+(\d{2}-\d{7}|\d{3}-\d{2}-\d{4})\s*$/;
const QBI_TOTAL_DEDUCTION = /^\s*39\s+Total qualified business income deduction/i;
const QBI_TAXABLE_BEFORE = /^\s*33\s+Taxable income before qualified business income deduction/i;
/** The thresholds the form prints in its own line labels: single first, married second. */
const QBI_THRESHOLD_LABEL = /Threshold\.\s*Enter\s*\$([\d,]+)[\s\S]{0,60}?\(\$([\d,]+)\s*if married/i;
const QBI_RANGE_LABEL = /Phase-in range\.\s*Enter\s*\$([\d,]+)[\s\S]{0,60}?\(\$([\d,]+)\s*if married/i;

/**
 * Fields section 199A names, recognisable from a business name.
 *
 * Health only, plus the professions whose names are unambiguous. Consulting, financial
 * services and brokerage are specified services too, but their words appear in the names of
 * ordinary real-estate and holding entities — "CAM INVESTMENT 501 LLC" is a property fund —
 * and a wrong flag here reads as an accusation about a deduction worth six figures.
 */
const SPECIFIED_SERVICE_NAME = /\b(?:medical|medicine|physician|surgic\w*|surgery|dental|dentistry|dermatolog\w*|radiolog\w*|anesthes\w*|orthoped\w*|orthodont\w*|pediatric\w*|cardiolog\w*|oncolog\w*|psychiatr\w*|psycholog\w*|chiropract\w*|veterinar\w*|optometr\w*|ophthalmolog\w*|patholog\w*|urolog\w*|neurolog\w*|clinic|hospital|law offices?|law group|attorneys?|counsellors? at law|accountancy|certified public account\w*)\b/i;
/** Structures built precisely so they are not specified services. */
const NOT_THE_PRACTICE = /\b(?:mso|management services organization|management company)\b/i;

/** Every business named in Part I of every Form 8995-A in the package. */
function qbiBusinesses(text) {
  const lines = linesOf(text);
  const out = [];
  let inPartI = false;
  for (const line of lines) {
    if (QBI_PART_I.test(line.trim())) { inPartI = true; continue; }
    if (!inPartI) continue;
    if (/^Part II\b/i.test(line.trim())) { inPartI = false; continue; }
    const row = QBI_BUSINESS_ROW.exec(line);
    if (!row) continue;
    const raw = row[2].trim();
    // Un nombre que se corto en la linea de arriba deja la fila con solo un identificador.
    if (/^(?:[0-9]{2}-[0-9]{7}|[0-9]{3}-[0-9]{2}-[0-9]{4})$/.test(raw)) continue;
    // A checked box prints an X of its own between the name and the number.
    const specifiedService = /\sX$/.test(raw);
    out.push({ column: row[1], name: raw.replace(/\sX$/, "").trim(), id: row[3], specifiedService });
  }
  return out;
}

/** The income above which a specified service business gets nothing, from the form itself. */
function qbiCeiling(text) {
  const threshold = QBI_THRESHOLD_LABEL.exec(text);
  const range = QBI_RANGE_LABEL.exec(text);
  if (!threshold || !range) return null;
  const married = parseAmount(threshold[2]);
  const spread = parseAmount(range[2]);
  if (married === null || spread === null) return null;
  // The married ceiling is the higher of the two the form prints, so a return above it is
  // above the ceiling whatever the filing status turns out to be.
  return round2(married + spread);
}

/**
 * A figure on a numbered line, never the line's own box number.
 *
 * An empty line prints its number and nothing else, so line 39 with no deduction on it reads
 * as $39. This file has now been bitten by that three times — the New York depreciation line,
 * the Form 8825 rental line, and here — so the guard is written once and called by name.
 */
function numberedLineAmount(text, pattern) {
  const line = lineMatching(text, pattern);
  if (!line) return null;
  const values = amountsOn(line);
  if (!values.length) return null;
...
function checkSpecifiedServiceQbi(text) {
  const deduction = numberedLineAmount(text, QBI_TOTAL_DEDUCTION);
  if (!deduction || deduction <= 0) return null;
  const taxable = numberedLineAmount(text, QBI_TAXABLE_BEFORE);
  const ceiling = qbiCeiling(text);
  if (taxable === null || ceiling === null || taxable <= ceiling) return null;

  const flagged = qbiBusinesses(text).filter((business) =>
    !business.specifiedService
    && SPECIFIED_SERVICE_NAME.test(business.name)
    && !NOT_THE_PRACTICE.test(business.name));
  if (!flagged.length) return null;

  const named = flagged.map((business) => `"${business.name}"`).join("; ");
  return {
    severity: "HIGH",
    category: "Qualified business income",
    title: "A business named for a specified service field is not flagged as one on Form 8995-A",
    detail: `Form 8995-A Part I lists ${named} with column (b), "check if specified service", left blank, and the return claims ${money(deduction)} of qualified business income deduction on taxable income of ${money(taxable)} — above the ${money(ceiling)} ceiling this form prints. Above that ceiling a specified service trade or business is excluded whole: its income, its W-2 wages and its UBIA all drop out, so the checkbox is worth the deduction rather than a part of it.`,
    action: "Confirm what each business actually does. If it performs services in health, law, accounting or another field section 199A names, tick column (b) and complete Schedule A (Form 8995-A) — at this income level that removes the business from the computation entirely. If it genuinely is not a specified service, the file should say why, because the name invites the question on examination.",
    authority: "IRC §199A(d)(2) and (d)(3); Reg. §1.199A-5(b); Form 8995-A Part I column (b) and Schedule A",
  };
}


/* ---------------------------------------------------------------------------
 * 13. Unrecaptured section 1250 gain a K-1 reports and the return does not carry.
 *
 * Gain attributable to depreciation already taken on real property is taxed at 25%, not at the
 * 20% that applies to the rest of a long-term gain. A partnership reports the partner's share
 * in box 20 with code AD, and the partner has to put it on Schedule D line 19 — which is also
 * what sends the return through the Schedule D Tax Worksheet instead of the Qualified
 * Dividends and Capital Gain Tax Worksheet, the one that knows nothing about the 25% band.
 *
 * On a real 1040 the K-1 for the sale of a medical practice reported "AD 97,475." and the
 * figure appears nowhere in the return: line 19 blank, line 20 answered "yes, both zero or
 * blank", and the whole gain taxed at 20%. The IRS matches box 20 codes, so this is both an
 * understatement and a visible one.
 * ------------------------------------------------------------------------- */

/** Box 20 code AD as a K-1 prints it: the code on its own, then the partner's share. */
const K1_CODE_AD = /^\s*AD\s+([\d,]+)\.?\s*$/;
/** Schedule D line 19 wraps; the amount lands after the repeated box number. */
const SCHEDULE_D_LINE_19 = /from line 18 of that worksheet/i;
/** Below this the rate difference is not worth a preparer opening the file. */
const UNRECAPTURED_FLOOR = 1000;

/** Every unrecaptured section 1250 gain the package reports to this taxpayer. */
function unrecapturedGainOnK1s(files) {
  const found = [];
  for (const file of Array.isArray(files) ? files : []) {
    const text = String((file && (file.fullText || file.text)) || "");
    for (const line of linesOf(text)) {
      const hit = K1_CODE_AD.exec(line);
      if (!hit) continue;
      const value = parseAmount(hit[1]);
...
function checkUnrecapturedGainNotCarried(text, files) {
  const reported = unrecapturedGainOnK1s(files);
  if (!reported.length) return null;
  const total = round2(reported.reduce((sum, value) => sum + value, 0));
  if (total < UNRECAPTURED_FLOOR) return null;

  // The return has to have a Schedule D for the line to be missing from.
  const line19 = lineMatching(text, SCHEDULE_D_LINE_19);
  if (!line19) return null;
  const values = amountsOn(line19);
  const last = values.length ? values[values.length - 1] : null;
  const onReturn = last === null || last === 19 ? 0 : last;
  if (onReturn > 0) return null;

  const each = reported.length > 1 ? ` across ${reported.length} K-1s` : "";
  return {
    severity: "HIGH",
    category: "Capital gain rate",
    title: "Unrecaptured section 1250 gain reported on a K-1 is not on Schedule D",
    detail: `A Schedule K-1 in this package reports ${money(total)} of unrecaptured section 1250 gain in box 20, code AD${each}, and Schedule D line 19 is blank. That gain is taxed at 25%, so the return has computed the tax on it at the long-term rate and understated it by the difference.`,
    action: "Put the code AD amount on Schedule D line 19 and complete the Unrecaptured Section 1250 Gain Worksheet. Line 19 is also what moves the return onto the Schedule D Tax Worksheet; with line 19 blank the return used the Qualified Dividends and Capital Gain Tax Worksheet, which has no 25% band at all. The IRS matches box 20 codes against the return.",
    authority: "IRC §1(h)(1)(E) and §1(h)(6); Schedule D (Form 1040) line 19 and the Unrecaptured Section 1250 Gain Worksheet; Schedule K-1 (Form 1065) box 20 code AD",
  };
}


/* ---------------------------------------------------------------------------
 * 14. Gain taken out of net investment income with nothing showing the calculation.
 *
 * Section 1411(c)(4) does not exclude the gain on selling a partnership interest; it limits
 * what goes into net investment income to the gain a deemed sale of the entity's own assets
 * would allocate to that partner. Working that out needs the entity's balance sheet, an asset
 * by asset split between what is used in a section 1411 trade or business and what is not, and
 * the transferor's share of each. It is a computation, not an election, and the proposed
 * regulation asks for it in writing.
 *
 * On a real 1040 line 5c removed $9,367,130 of a $9,548,612 gain — 98% of it — and the package
 * held no deemed-sale schedule, no reference to section 1411 property, nothing. At 3.8% the
 * line is worth $355,951, which makes it the largest single number on the return and the one
 * with the least behind it.
 * ------------------------------------------------------------------------- */

/** Form 8960 line 5c: the label wraps, so the amount rides on the continuation. */
const NIIT_DISPOSITION_ADJUSTMENT = /^\s*c?\s*stock \(see instructions\)|^\s*5\s*c\s+Adjustment from disposition/i;
const NIIT_GAIN_FROM_DISPOSITION = /^\s*5\s*a\s+Net gain or loss from disposition of property/i;
/**
 * Evidence that the section 1411 deemed sale was actually worked out.
 *
 * "Deemed sale" alone is not evidence. Form 8308 heads its own columns "Partnership-level
 * deemed sale gain (loss)" and "Partner-level deemed sale gain (loss)" — that is the section
 * 751 computation, a different provision — so any package containing a Form 8308 looked as
 * though it carried a 1411 schedule, and the check went quiet on the very return that needed
 * it. The phrase only counts when it is near a mention of 1411.
 */
const DEEMED_SALE_EVIDENCE = /1\.1411-7|1411-7|section 1411 property|optional simplified reporting|non-?section 1411 (?:trade or business )?assets/i;
const DEEMED_SALE_NEAR_1411 = /deemed sale[\s\S]{0,120}?1411|1411[\s\S]{0,120}?deemed sale/i;
/** Below this the adjustment is not worth a schedule. */
const NIIT_ADJUSTMENT_FLOOR = 100000;
/** The rate the adjustment is worth. */
const NIIT_RATE = 0.038;

function checkNiitDispositionAdjustment(text, files) {
  const line = lineMatching(text, NIIT_DISPOSITION_ADJUSTMENT);
  if (!line) return null;
  const values = amountsOn(line);
  if (!values.length) return null;
  const adjustment = Math.abs(values[values.length - 1]);
  if (adjustment < NIIT_ADJUSTMENT_FLOOR) return null;

  // Only the whole package can say whether the calculation exists; it may live in a schedule
  // rather than on the form.
  const everything = [text, ...(Array.isArray(files) ? files : []).map((file) => String((file && (file.fullText || file.text)) || ""))].join("\n");
  if (DEEMED_SALE_EVIDENCE.test(everything) || DEEMED_SALE_NEAR_1411.test(everything)) return null;

  const gain = amountFrom(text, NIIT_GAIN_FROM_DISPOSITION);
  const share = gain && gain > 0 ? Math.round((adjustment / Math.abs(gain)) * 100) : null;
  const removed = share === null ? "" : ` — ${share}% of the ${money(Math.abs(gain))} on line 5a`;
  return {
    severity: "HIGH",
    category: "Net investment income tax",
    title: "Gain removed from net investment income with no deemed-sale calculation in the package",
    detail: `Form 8960 line 5c takes ${money(adjustment)} out of net investment income${removed}, worth ${money(round2(adjustment * NIIT_RATE))} of tax at 3.8%. Nothing in the package shows how that figure was arrived at: there is no deemed-sale schedule and no reference to section 1411 property anywhere in the return or the supporting documents.`,
    action: "Attach the computation. Section 1411(c)(4) limits what enters net investment income to the gain a deemed sale of the entity's assets would allocate to this partner, which takes the entity's balance sheet, an asset-by-asset split between section 1411 and non-section 1411 use, and the transferor's share of each. An adjustment of this size with nothing behind it is the first thing an examiner asks for, and the partnership is the only place the data exists.",
    authority: "IRC §1411(c)(4); Prop. Reg. §1.1411-7 and its information statement; Form 8960 line 5c",
  };
}
...
function runReturnConsistencyChecks(files, meta = {}) {
  ...
  return [
    ...
    checkScheduleM1TiesToBooks(text, files, Number.isFinite(taxYear) ? String(taxYear) : null),
    checkClosingInventoryMissing(text, files),
    checkSpecifiedServiceQbi(text),
    checkUnrecapturedGainNotCarried(text, files),
    checkNiitDispositionAdjustment(text, files),
  ].filter(Boolean);
}
...
module.exports = {
  ...
  checkSpecifiedServiceQbi,
  qbiBusinesses,
  qbiCeiling,
  numberedLineAmount,
  checkUnrecapturedGainNotCarried,
  unrecapturedGainOnK1s,
  checkNiitDispositionAdjustment,
  bookNetIncome,
  bookInventory,
  jurisdictionsFiled,
  accountingMethod,
