...
function enforceNumericVerdicts(review, returnType, files) {
  ...
  const evidence = verifyTieOutEvidence(unsupported.rows, returnType, files);
  ...
  return {
    ...
    unsupported: unsupported.flagged + evidence.flagged,
...
function verifyTieOutEvidence(rows, returnType, files) {
  ...
  const out = list.map((row) => {
    ...
    // 1. The support is a stated sum — re-add it. A chain that does not reach its own
    //    total means the components were not actually totalled.
    const derivation = parseDerivation(note);
    if (derivation) {
      const sum = round2(derivation.parts.reduce((a, b) => a + b, 0));
      const target = derivation.stated === null ? wp : derivation.stated;
      if (Math.abs(sum - target) >= 0.01) {
        return downgrade(`the amounts listed in this note add to ${sum.toFixed(2)}, not ${Number(target).toFixed(2)}. Re-add the support.`);
      }
      if (Math.abs(sum - wp) >= 0.01) {
        return downgrade(`the note derives ${sum.toFixed(2)} but the support column shows ${wp.toFixed(2)}.`);
      }
      return row; // arithmetic shown and it holds
    }

    // 2. No derivation shown, so the figure itself must be readable on a support document.
...
function auditDocumentCoverage(review, files) {
  ...
  return { coverage, unreviewed: coverage.filter((c) => c.status !== "REVIEWED") };
}
...
module.exports = {
  ...
  verifyTieOutEvidence, auditDocumentCoverage, textCitesFile, amountAppearsInText, parseDerivation,
};
