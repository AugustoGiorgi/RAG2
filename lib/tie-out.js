 ...
 */
/* ---------------------------------------------------------------------------
 * The return side of each tie-out, read by code.
 *
 * The model copied the return figure for each row, and a misread there (a line from the
 * state return, the prior year, a worksheet) turned a correct return into an out-of-balance
 * row or, worse, hid a real difference. The return's own lines are read with fixed rules
 * (return-facts / entity-facts); where code has the figure, it replaces the model's.
 * ------------------------------------------------------------------------- */

const CODE_LINES_1040 = {
  form1040l1a: "1a", form1040l2b: "2b", form1040l3a: "3a", form1040l3b: "3b", form1040l7: "7",
  form1040l11: "11", form1040l15: "15", form1040l24: "24", form1040l25d: "25d", form1040l26: "26", form1040l33: "33",
};

function returnSideFromCode(rows, returnType, files) {
  const list = Array.isArray(rows) ? rows : [];
  if (!list.length || !Array.isArray(files) || !files.length) return { rows: list, changed: 0 };
  // Lazy: these modules read the package, and tie-out.js is also loaded where they are not needed.
  const { splitReturns } = require("./prior-year-bridge");
  const rf = require("./return-facts");
  const ef = require("./entity-facts");
  const { current } = splitReturns(files.map((f) => ({ ...f, reviewRole: f.reviewRole || f.role })), {});
  if (!current) return { rows: list, changed: 0 };
  const text = rf.textOf(current);
  const known = {};
  if (/^1040/.test(String(returnType || "").replace(/\s+/g, "")) && rf.isForm1040(text)) {
    const lines = rf.form1040Lines(text);
    for (const [key, line] of Object.entries(CODE_LINES_1040)) if (lines[line] !== null && lines[line] !== undefined) known[key] = lines[line];
  }
  // En una entidad las filas no tienen numero de linea en el rotulo ("Schedule M-2 / retained
  // earnings roll-forward"), asi que cada fila del checklist se reconoce por su texto. El M-2 es
  // el que motivo esto: el modelo armo su propio arrastre —inicio mas resultado del año— y lo
  // declaro TIE, sin ver que el M-2 traia otros aumentos por $4,2 millones. Con las dos cifras
  // inventadas por el mismo lado, el control aritmetico no podia notar nada.
  const entityRows = [];
  const type = ef.entityType(text);
  if (type) {
    const lines = ef.entityLines(text);
    const m2 = type === "1120-S" ? lines.m2EndingFirstColumn : lines.m2Ending;
    const push = (re, value) => { if (value !== null && value !== undefined) entityRows.push([re, value]); };
    push(/gross receipts/i, lines.grossReceipts);
    push(/cost of goods sold/i, lines.costOfGoodsSold);
    push(/total deductions/i, lines.totalDeductions);
    push(/ordinary business income/i, type === "1120" ? lines.incomeBeforeNol : lines.ordinary);
    push(/total assets/i, lines.totalAssets && lines.totalAssets.ending);
    push(/total liabilities/i, lines.totalLiabilitiesAndCapital && lines.totalLiabilitiesAndCapital.ending);
    push(/schedule m-1|book income to taxable/i, lines.m1Closing);
    push(/schedule m-2|retained earnings roll/i, m2);
  }
  const codeValue = (lineItem) => {
    const key = canonicalLineKey(lineItem);
    if (known[key] !== undefined) return known[key];
    const row = entityRows.find(([re]) => re.test(String(lineItem || "")));
    return row ? row[1] : undefined;
  };
  let changed = 0;
  const out = list.map((row) => {
    if (!row || typeof row !== "object") return row;
    const value = codeValue(row.lineItem);
    if (value === undefined) return row;
    const given = parseAmount(row.returnAmount);
    if (given !== null && Math.abs(given - value) < TIE_THRESHOLD) return row;
    changed += 1;
    return {
      ...row,
      returnAmount: value,
      note: appendNote(row.note, given === null
        ? "Return amount read from the return by code."
        : `Return amount read from the return by code (the review had ${given.toFixed(2)}).`),
    };
  });
  return { rows: out, changed };
}
...
function enforceNumericVerdicts(review, returnType, files) {
  ...
  const fromCode = returnSideFromCode(review.tieOutResults, returnType, files);
  const tie = enforceTieOutVerdicts(fromCode.rows);
  ...
  const evidence = verifyTieOutEvidence(derived.rows, returnType, files, review.verifiedItems);
  ...
  return {
    ...
    corrections: tie.changed + bs.changed + rollup.flagged + fromCode.changed,
    returnSideFromCode: fromCode.changed,
...
// Los titulos federales empiezan con "U.S.". Van primero porque las estatales copian el resto:
// "Arizona S Corporation Income Tax Return" (el 120S) aparece antes que el federal en un paquete
// de Arizona y hacia pasar un 1120-S por 1120.
const FEDERAL_TITLES = [
  { type: "990", re: /return\s+of\s+organization\s+exempt\s+from\s+income\s+tax/i },
  { type: "1120-S", re: /u\.\s?s\.\s+income\s+tax\s+return\s+for\s+an\s+s\s+corporation/i },
  { type: "1065", re: /u\.\s?s\.\s+return\s+of\s+partnership\s+income/i },
  { type: "1041", re: /u\.\s?s\.\s+income\s+tax\s+return\s+for\s+estates\s+and\s+trusts/i },
  { type: "1040", re: /u\.\s?s\.\s+individual\s+income\s+tax\s+return/i },
  { type: "1120", re: /u\.\s?s\.\s+corporation\s+income\s+tax\s+return/i },
];

function earliestSignature(body, signatures) {
  let best = null;
  for (const { type, re } of signatures) {
...
function detectReturnTypeFromText(text) {
  ...
  if (!body) return "";
  const federal = earliestSignature(body, FEDERAL_TITLES);
  if (federal) return federal.type;
  const best = earliestSignature(body, RETURN_SIGNATURES);
...
/**
 * Un termino de una cadena de suma.
 *
 * Los parentesis tienen que estar balanceados: o el importe entero va entre parentesis, que
 * en un formulario significa negativo, o no hay parentesis. Sin esa condicion la cadena se
 * derramaba mas alla del cierre: sobre un 1040 real "(Line 1a $1,470,470) + 2b interest" se
 * leyo como 1.470.470 + 2, y "2b" es una referencia de linea, no dos dolares.
 *
 * Y ningun termino puede venir pegado a una letra, por lo mismo: 2b, 3a y 1z son lineas.
 *
 * El signo menos es un negativo, y vale en cualquier lugar donde lo escriba un contador:
 * "-18,438 + -2,515", "$-1,234", "-$1,234". La version que balanceo los parentesis se olvido
 * de el, y las cadenas con negativos dejaron de auditarse sin que nada lo notara. Lo unico que
 * no puede ser un signo es un guion pegado a una letra o a un digito: en "W-2 + 1099" el 2 es
 * parte del nombre del formulario, no menos dos. Por eso ni el termino ni su signo pueden
 * venir inmediatamente despues de una letra, un digito o un separador de miles.
 */
const CHAIN_TERM = String.raw`(?<![A-Za-z0-9.,$-])(?:-\s?)?(?:\$\s?)?-?(?:\((?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d+)?\)|(?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d+)?)(?![A-Za-z0-9])`;
const CHAIN_RE = new RegExp(`${CHAIN_TERM}(?:\\s*\\+\\s*${CHAIN_TERM}){1,}`, "g");
const STATED_AFTER = new RegExp(`^\\s*=\\s*(${CHAIN_TERM})`);
/** Un total escrito ANTES de su detalle: "sum of W-2 Box 1 is $377,814.01 ($351,200 + ...)". */
const STATED_BEFORE = new RegExp(`\\$\\s?((?:\\d{1,3}(?:,\\d{3})+|\\d+)(?:\\.\\d+)?)[^$+=]{0,40}[(:]\\s*$`);

/**
 * TODAS las cadenas de suma que muestra una nota, con el total que cada una declara.
 *
 * En plural, y esa es la correccion. La version anterior devolvia la PRIMERA y la trataba como
 * la derivacion de la fila. En un informe real de un 1040 eso marco cuatro lineas correctas
 * como no verificadas: en los dividendos ordinarios agarro "$3+3+4+4" — cuatro fondos Franklin
 * de la lista de catorce pagadores — y comparo esos $14 contra los $16.888 de la columna,
 * mientras la nota terminaba con la suma entera de los catorce dando $16.888 exacto.
 *
 * Una nota de tie-out no es una cuenta: es prosa con varias cuentas adentro, subtotales entre
 * parentesis incluidos. Cual de ellas es "el total de la fila" solo se sabe si la nota lo dice.
 *
 * Devuelve [{ parts, sum, stated }], vacio si no hay ninguna.
 */
function parseDerivations(note) {
  const text = String(note || "");
  const out = [];
  for (const chain of text.matchAll(CHAIN_RE)) {
    const parts = chain[0].split("+").map((p) => parseAmount(p)).filter((n) => n !== null);
    if (parts.length < 2) continue;
    const after = text.slice(chain.index + chain[0].length).match(STATED_AFTER);
    const before = after ? null : text.slice(Math.max(0, chain.index - 80), chain.index).match(STATED_BEFORE);
    const stated = after ? parseAmount(after[1]) : (before ? parseAmount(before[1]) : null);
    out.push({ parts, sum: round2(parts.reduce((a, b) => a + b, 0)), stated });
  }
  return out;
}

/** Si la nota muestra su propia aritmetica. Se conserva porque otros pasos preguntan solo eso. */
function parseDerivation(note) {
  const all = parseDerivations(note);
  return all.length ? { parts: all[0].parts, stated: all[0].stated } : null;
}
...
function auditDerivations(rows) {
  ...
  const out = (Array.isArray(rows) ? rows : []).map((row) => {
    ...
    const chains = parseDerivations(row.note);
    if (!chains.length) return row;
    ...
    // Primero, cada cadena que declara su propio total tiene que llegar a el. Esto es lo que
    // atrapo el caso que motivo el chequeo: "sum of W-2 Box 1 is $377,814.01 ($351,200 +
    // $2,517.85 + $12,048.08)", donde los tres terminos dan $365.765,93 y el total declarado
    // contaba un W-2 dos veces.
    for (const chain of chains) {
      if (chain.stated !== null && Math.abs(chain.sum - chain.stated) >= 0.01) {
        return downgrade(`the amounts listed in this note add to ${chain.sum.toFixed(2)}, not ${chain.stated.toFixed(2)}. Re-add the support and confirm no document was counted twice or left out.`);
      }
    }

    // Despues, contra la columna de soporte — y solo con los totales que la nota DECLARA.
    //
    // Una cadena suelta, sin total escrito, no afirma nada sobre la fila: es un subtotal en
    // medio de la prosa. Compararla contra la columna es lo que marcaba como no verificadas
    // lineas que ataban exacto. Si la nota no declara ningun total, no hay nada que contrastar
    // y la fila queda como estaba; el que se ocupa de una cifra sin respaldo es
    // verifyTieOutEvidence, que la busca en los documentos.
    const stated = chains.filter((chain) => chain.stated !== null);
    if (!stated.length) return row;
    const wp = parseAmount(row.workpaperAmount);
    if (wp === null) return row;
    if (stated.some((chain) => Math.abs(chain.stated - wp) < TIE_THRESHOLD)) return row;
    const totals = stated.map((chain) => chain.stated.toFixed(2)).join(", ");
    return downgrade(stated.length === 1
      ? `this note states a total of ${totals}, which is not the ${wp.toFixed(2)} in the support column. Reconcile the two before relying on this line.`
      : `this note states totals of ${totals}, and none of them is the ${wp.toFixed(2)} in the support column. Reconcile the two before relying on this line.`);
  });
 ...
 */
/**
 * Lo que el modelo leyo de los PDF escaneados, sacado de sus lineas "SCANNED:".
 *
 * Un escaneado no tiene texto extraible: el modelo lo recibe como imagen y el prompt le exige
 * transcribir cada pagina en una linea "SCANNED: <archivo> — p1 ...". Sin esto, la busqueda de
 * evidencia solo miraba el texto extraido, y toda cifra que salia de un escaneado quedaba
 * NOT VERIFIED aunque estuviera ahi: en un 1040 real la retencion de $8,911 del W-2 escaneado
 * salio "not found in any supporting document" mientras el mismo informe transcribia el W-2
 * con esa cifra en el box 2.
 *
 * Devuelve [{ name, text }].
 */
function scannedReadings(verifiedItems) {
  return (Array.isArray(verifiedItems) ? verifiedItems : [])
    .map((line) => String(line || ""))
    .filter((line) => /^\s*SCANNED:/i.test(line))
    .map((line) => {
      const body = line.replace(/^\s*SCANNED:\s*/i, "");
      const name = (body.match(/^(.+?)\s+[—–-]\s+p(?:age)?\s*\d/i) || [])[1] || body.slice(0, 60);
      return { name: name.trim(), text: body };
    });
}

function verifyTieOutEvidence(rows, returnType, files, verifiedItems) {
  const list = Array.isArray(rows) ? rows : [];
  ...
  const scanned = scannedReadings(verifiedItems);
  const externalKeys = new Set(required.filter((r) => r.external).map((r) => canonicalLineKey(r.label)));
  let flagged = 0;
  ...
  const out = list.map((row) => {
    ...
    // Leida de un escaneado: la fila queda como esta, pero dice de donde salio. Es la lectura
    // de una imagen hecha por el mismo modelo que armo la fila, asi que el revisor tiene que
    // saber que la confirmacion es a ojo, no contra un texto.
    const fromScan = scanned.find((s) => amountAppearsInText(wp, s.text));
    if (fromScan) {
      return { ...row, note: appendNote(row.note, `Figure read from a scanned document (${fromScan.name}); confirm it against the image.`) };
    }
...
module.exports = {
  enforceNumericVerdicts, enforceTieOutVerdicts, enforceBalanceSheetVerdict, parseAmount, TIE_THRESHOLD, NOT_VERIFIED, scannedReadings,
  returnSideFromCode,
  ...
  verifyTieOutEvidence, auditDocumentCoverage, auditDerivations, textCitesFile, amountAppearsInText, parseDerivation, parseDerivations,
};
