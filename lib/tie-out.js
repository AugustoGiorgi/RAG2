...
function returnSideFromCode(rows, returnType, files) {
  ...
  if (/^1040/.test(String(returnType || "").replace(/\s+/g, "")) && rf.isForm1040(text)) {
    ...
    if (type === "1065" || type === "1120-S") {
      const lines = ef.entityLines(text);
      if (lines.ordinary !== null && lines.ordinary !== undefined) known.ordinaryincome = lines.ordinary;
      if (lines.totalAssets && lines.totalAssets.ending !== null) known.schedulella = lines.totalAssets.ending;
    }
  }
  const codeKey = (lineItem) => {
    const key = canonicalLineKey(lineItem);
    if (known[key] !== undefined) return key;
    const label = String(lineItem || "").toLowerCase();
    if (known.ordinaryincome !== undefined && /ordinary business income/.test(label)) return "ordinaryincome";
    if (known.schedulella !== undefined && /schedule l\b.*total assets/.test(label)) return "schedulella";
    return null;
  };
  let changed = 0;
  const out = list.map((row) => {
    if (!row || typeof row !== "object") return row;
    const key = codeKey(row.lineItem);
    if (!key) return row;
