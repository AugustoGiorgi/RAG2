    ...
    if (headerIdx >= 0) {
      for (let r = headerIdx + 1; r < guide.rows.length; r++) {
        ...
        if (Math.abs(finAmt) >= 1) {
          const key = Math.round(finAmt * 100);
          for (const target of targets) {
            const matches = target.index.get(key);
            if (matches && matches.length === 1) {
              row[2] = { formula: `${sheetRef(target.sheet)}!${matches[0]}`, value: finAmt };
              break;
            }
          }
        }
...
module.exports = { canonicalizeWorkbookSheets, injectFinancialStatementFormulas, linkEntryGuideToWorkpaper, CANONICAL_SHEET_RULES };
