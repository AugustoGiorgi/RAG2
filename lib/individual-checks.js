...
const { qbiBusinesses } = require("./return-consistency-checks");
...
function checkStandardDeduction(ctx) {
  ...
  const status = ctx.header && ctx.header.filingStatus;
  const b = ctx.boxes;
  if (!table || !status || !b || !b.anyX || b.dependentOfAnother || b.noStandard) return [];
...
function moreDocumentChecks(ctx) {
  ...
  const m = docsOf("1098");
  const a = rf.scheduleA(ctx.text);
  // Schedule E linea 12 con importe (el "12" del numero de linea solo no cuenta).
  const rentalMortgage = rf.linesOf(ctx.text).some((l) => /Mortgage interest paid to banks, etc\./i.test(l) && /\s12\s+[\d,]{3,}/.test(l));
  if (m.length && a && !(num(a.mortgage1098) > 0) && !rentalMortgage) {
...
/* ---------------------------------------------------------------------------
 * Sueldo razonable visto desde el 1040
 *
 * La S corp que paga poco sueldo y reparte el resto es el ajuste mas comun del IRS sobre estas
 * estructuras, y el reviewer del 1040 casi nunca tiene la declaracion de la entidad a mano. No
 * hace falta: el 8995-A imprime, por cada negocio, el ingreso calificado y la parte del socio
 * en los sueldos W-2 de ese mismo negocio, y el Schedule E dice cual de esos negocios es una S
 * corporation. Las dos cifras son la misma proporcion cualquiera sea el porcentaje de
 * participacion, asi que la comparacion vale sin saber cuanto tiene el cliente.
 * ------------------------------------------------------------------------- */

/** Los importes de un renglon del 8995-A, uno por columna (A, B, C). */
function qbiColumnAmounts(lines, label, lineNo) {
  const i = lines.findIndex((l) => label.test(l));
  if (i < 0) return [];
  for (let j = i; j < Math.min(lines.length, i + 3); j += 1) {
    const m = new RegExp(`(?:^|\\s)${lineNo}\\s+((?:-?\\(?[\\d,]+\\)?\\.?\\s*)+)$`).exec(lines[j]);
    if (!m) continue;
    return (m[1].match(/-?\(?[\d,]+\)?/g) || []).map((v) => Number(v.replace(/[(),]/g, "")) * (/^\(/.test(v) ? -1 : 1));
  }
  return [];
}

/** Las entidades del Schedule E Parte II con su tipo: "A EXAMPLE LLC S 00-0000000 X". */
function scheduleEEntities(lines) {
  const out = [];
  for (const line of lines) {
    const m = /^\s*([A-D])\s+(.{3,60}?)\s+([PS])\s+(\d{2}-\d{7})\b/.exec(line);
    if (m) out.push({ column: m[1], name: m[2].trim(), type: m[3], ein: m[4] });
  }
  return out;
}

function checkReasonableCompensationFromK1(ctx) {
  const lines = rf.linesOf(ctx.text);
  const businesses = qbiBusinesses(ctx.text);
  if (!businesses.length) return [];
  const income = qbiColumnAmounts(lines, /Qualified business income from the trade, business, or/i, "2");
  const wages = qbiColumnAmounts(lines, /Allocable share of W-2 wages from the trade, business, or/i, "4");
  if (!income.length || income.length !== wages.length) return []; // lineas 4 a 12 salteadas: no se sabe
  const entities = scheduleEEntities(lines);
  const out = [];
  businesses.forEach((business, index) => {
    const entity = entities.find((e) => e.ein && e.ein === business.id);
    if (!entity || entity.type !== "S") return;
    const qbi = income[index];
    const paid = wages[index];
    if (!(qbi >= 75000) || paid === undefined || paid > qbi * 0.25) return;
    out.push(finding("MEDIUM", "Shareholder compensation", "Form 8995-A — S corporation income with little or no W-2 wages",
      `${business.name} is an S corporation on Schedule E Part II. Form 8995-A reports ${fmt(qbi)} of qualified business income from it and ${paid ? fmt(paid) : "no"} allocable W-2 wages${paid ? ` (${Math.round((paid / qbi) * 100)}% of the income)` : ""}.`,
      "Ask for the corporation's Form 1120-S line 7 and the officer's W-2. A shareholder who works in the business must be paid reasonable wages before distributions; the IRS recharacterizes distributions as wages with payroll tax, interest and penalty, and the wage figure also caps the QBI deduction.",
      "IRC §§3121, 1366, 199A(b)(2); Rev. Rul. 74-44", "Form 8995-A Part I and lines 2 and 4; Schedule E Part II, column (b)."));
  });
  return out;
}
...
const CHECKS = [
  documentAmountChecks, checkReasonableCompensationFromK1,
...
module.exports = {
  ...
  checkSelfEmploymentTax, checkQbiDeduction, checkSeniorDeduction, checkChildCredits, moreDocumentChecks, documentAmountChecks, checkStateMove, checkReasonableCompensationFromK1, qbiColumnAmounts, scheduleEEntities,
  digitDistance,
};
