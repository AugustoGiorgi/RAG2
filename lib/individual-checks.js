...
function checkResidencyAgainstDocuments(ctx) {
  ...
  return [finding("MEDIUM", "State residency", "Residency — documents place the taxpayer in another state",
    strong.map(([state, list]) => `${new Set(list.map((d) => d.doc)).size} documents for the year are addressed to the taxpayer in ${state} (${[...new Set(list.map((d) => `${d.doc} — ${d.city}`))].join("; ")}), while the return gives a ${home} address.`).join(" "),
...
module.exports = {
  ...
  checkSelfEmploymentTax, checkQbiDeduction, checkSeniorDeduction, checkChildCredits, moreDocumentChecks,
  digitDistance,
};
