...
const M1_LINES = [
  ...
  { key: "ownerHealthcare", label: "Add back: Owner healthcare premiums (not entity-deductible; SE health on 1040)" },
...
function buildM1Sheet(reconciliation = {}, entityType = "") {
  ...
  // Anti-double-count guard: the subtotals add Net Income + all AJEs + all m1 keys, so if the
  // model put the asset-sale income removal BOTH as an AJE and in m1.assetSaleIncomeRemoval it
  // would be subtracted twice. Drop an AJE that matches assetSaleIncomeRemoval to the cent AND
  // whose label clearly refers to an asset sale (both conditions → safe: not a coincidental match).
  const assetRemoval = round2(num(m1.assetSaleIncomeRemoval));
  if (assetRemoval !== 0) {
    ajes = ajes.filter((a) => {
      const amt = round2(num(a && a.amount));
      const label = String((a && a.label) || "");
      const looksLikeAssetSale = /asset\s*sale|sale\s*of\s*asset|proceeds|4797|1231|disposition|gain\s*on\s*sale/i.test(label);
      return !(Math.abs(amt - assetRemoval) <= 0.01 && looksLikeAssetSale);
    });
  }
