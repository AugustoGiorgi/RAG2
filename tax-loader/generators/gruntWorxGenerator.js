...
/**
 * Format a Social Security Number as XXX-XX-XXXX (with dashes).
 * Drake stores SSNs WITH dashes on the TP screen (e.g. "117-52-7660").
 * IMPORTGW.DLL compares RecipientIdNo/EmployeeSSN against the return's
 * stored SSN using a direct string match — sending "117527660" (no dashes)
 * when Drake has "117-52-7660" causes every TSJ match to fail and Drake
 * silently discards all records. Always emit dashes so the comparison succeeds.
 */
function formatSsn(v) {
  const digits = String(v || '').replace(/[^0-9]/g, '');
  if (digits.length === 9) return `${digits.slice(0, 3)}-${digits.slice(3, 5)}-${digits.slice(5)}`;
  return digits; // passthrough for non-standard lengths (EINs, partial SSNs)
}
...
function buildW2Record(w) {
  ...
  return record([
    field('EmployeeSSN',    'Social Security Number',  formatSsn(w.ssn || w.employee_ssn || w.box_a || '')),
...
function buildIntRecord(r) {
  ...
  return record([
    field('RecipientIdNo',   'Social Security Number', formatSsn(r.ssn || r.recipient_ssn || '')),
...
function buildSsaRecord(r) {
  ...
  return record([
    field('BeneficiarySSNo',  'Social Security Number', formatSsn(r.ssn || r.beneficiary_ssn || '')),
