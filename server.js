...
function buildPreparerContent(payload) {
  ...
  const content = [{
    ...
    text: [
      ...
      "",
      "EXCEL FORMULAS — keep them MINIMAL: use a formula ONLY for subtotal and total rows.",
      ...
      "- For a SUBTOTAL or TOTAL row, output an Excel SUM formula STRING that begins with '=' and sums the exact component cells in A1 notation — do NOT hard-code the total. Examples: \"=SUM(B6:B12)\", or for a total of subtotals \"=B13+B30+B45\".",
      "- Do NOT use formulas for difference/variance columns, for individual line items, or for tie-out checks. Put plain computed numbers in those cells. Do NOT use cross-sheet references (no 'Sheet'!Cell); within a sheet only.",
      "- Cell addressing: the FIRST row you output in a sheet is Excel row 1, the SECOND row is row 2, and so on. The first element of each row is column A, the second is column B, the third is column C, etc. Count rows and columns carefully so each SUM range covers only the intended component rows and nothing else, in the correct column.",
      "- If you are not certain a SUM range is exactly right, output the computed number instead of a formula. Never write a formula that points at an empty, text, or non-existent cell.",
      "",
