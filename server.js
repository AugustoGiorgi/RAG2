...
function buildPreparerContent(payload) {
  ...
  const content = [{
    ...
    text: [
      ...
      "Every sheet.rows value must be an array of rows, and every row must be an array of primitive cell values (numbers, text, or formula strings).",
      ...
      "",
      "EXCEL FORMULAS (so the preparer can change one amount and have everything recompute):",
      "- Output every base/source amount as a plain JSON number (e.g., 465.20 or -6410.66), NOT as a string. Do not put $ signs, commas, or parentheses inside amount cells; use a real negative number for negatives.",
      "- For any cell that is a SUBTOTAL, TOTAL, DIFFERENCE, VARIANCE, or TIE-OUT, output an Excel formula STRING that begins with '=' and references the exact cells it aggregates in A1 notation — do NOT hard-code the computed number. Examples: \"=SUM(B6:B12)\", \"=B8+B9\", \"=B10-C10\", \"=B25-B40\".",
      "- Cell addressing: the FIRST row you output in a sheet is Excel row 1, the SECOND row is row 2, and so on. The first element of each row is column A, the second is column B, the third is column C, etc. Count rows and columns carefully so each range covers only the intended component rows and nothing else.",
      "- The book-to-tax bridge, Schedule M-1, Schedule M-2, COGS (Form 1125-A), and the Tax Return Summary must use formulas that chain from their components, so editing any input updates the result automatically.",
      "- A difference/variance column (current year vs prior year) must be a formula like \"=B7-C7\", not a hard number.",
      "- Cross-sheet references are encouraged when a value comes from another sheet: use 'Sheet Name'!Cell, e.g. \"='Profit and Loss'!B45\". The sheet name must exactly match a sheet you output.",
      "- Only DERIVED cells use formulas. Never wrap a raw source amount in a formula, and never write a formula that points at an empty, text, or non-existent cell. If you are not certain a reference is correct, output the computed number instead.",
      "",
