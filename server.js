...
function buildPreparerContent(payload) {
  ...
  const content = [{
    ...
    text: [
      ...
      '    "m1": { "meals50": number, "entertainment": number, "penalties": number, "politicalLobbying": number, "officerLifeInsurance": number, "federalIncomeTax": number, "charitable": number, "ownerHealthcare": number, "homeOffice": number, "creditCardRewards": number, "taxExemptInterest": number, "depreciationBookVsTax": number, "sec179Bonus": number, "gainLossBookVsTax": number, "section163j": number, "otherPermanent": number, "otherTiming": number },',
